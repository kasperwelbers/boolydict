...
search_query <- function(df, queries, text_col='text', context_col=NULL, index_col=NULL, mode = c('hits','terms','contexts'), keep_longest=TRUE, as_ascii=F, use_wildcards=TRUE){
  mode = match.arg(mode)
  ...
  hits = perform_query_search(df, queries, mode, text_col, index_col, context_col, as_ascii, use_wildcards)
...
perform_query_search <- function(df, queries, mode, text, index, context, as_ascii, use_wildcards) {
  hit_id = NULL
  ...
  dict_results = get_dict_results(df, queries$lookup_terms, text, index=index, context=context, as_ascii=as_ascii, use_wildcards=use_wildcards)
