...
search_query <- function(df, queries, text_col='text', context_col=NULL, index_col=NULL, mode = c('hits','terms'), keep_longest=TRUE, as_ascii=F){
  mode = match.arg(mode)
  ...
  hits = perform_query_search(df, queries, mode, text_col, index_col, context_col, as_ascii)
